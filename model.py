...
class Model(object):
    ...
    def build(self, is_train=True):
        ...
        def build_loss(d_real, d_real_logits, d_fake, d_fake_logits, label, real_image, fake_image):
            ...
            d_loss_real = tf.nn.softmax_cross_entropy_with_logits_v2(logits=d_real_logits, labels=real_label)
            d_loss_fake = tf.nn.softmax_cross_entropy_with_logits_v2(logits=d_fake_logits, labels=fake_label)
        ...
        def D(img, scope='Discriminator', reuse=True):
            with tf.variable_scope(scope, reuse=reuse) as scope:
                ...
                d_4 = slim.fully_connected(
                    tf.reshape(d_3, [self.batch_size, -1]), int(n+1), scope='d_4_fc', activation_fn=None)
